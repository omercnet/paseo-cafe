...
export function PluginGalleryPage({ entry, theme, compact, onBack }: PluginGalleryPageProps) {
  const styles = useMemo(
    () => ({
      ...
      title: { color: theme.colors.foreground, fontSize: compact ? 20 : 24, fontWeight: "700" as const },
      images: { gap: 20 },
    }),
    [theme, compact]
  )
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.images}>
          {entry.images.map((image) => (
            <AspectImage key={image} uri={image} backgroundColor={theme.colors.surface2} />
          ))}
        </View>
      </ScrollView>
    </View>
